...
x1 = scale(as.matrix(cancer[, 2:11])) # Here I add scale() to avoid float number overflow
x2 = scale(as.matrix(cancer[, 2:21]))
x3 = scale(as.matrix(cancer[, 2:31]))
...
logisticstuff(x1, y, rep(2, 11))
logisticstuff(x2, y, rep(2, 21))
logisticstuff(x3, y, rep(2, 31))
```
...
nr1 = NewtonRaphson(x1, y, logisticstuff, rep(2,11))
nr1
nr2 = NewtonRaphson(x2, y, logisticstuff, rep(1,21))
nr3 = NewtonRaphson(x3, y, logisticstuff, rep(1,31))
```
...
ans = data.frame(nr1)
=======
ans = data.frame(NewtonRaphson(x1, y, logisticstuff, rep(0.5,11)))
>>>>>>> 94b8b680139344fcd058a898e37a42ce11f94e30
...
ans2 = data.frame(nr2)
colnames(ans2) = c("iteration", "log likelihood")
ggplot(ans2, aes(x = iteration, y = `log likelihood`)) +
  geom_point(size = 0.5) +
  geom_line()
