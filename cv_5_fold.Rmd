...
Cross-validation is done by dividing the data into a training set and a test set. The test set is data that is independent from the training and is not involved in the training at all, and is used for the evaluation of the final model. During the training process, the problem of overfitting often occurs, that is, the model can match the training data well, but cannot predict the data outside the training set well. If the test data is used to adjust the model parameters at this point, it is equivalent to knowing some of the information of the test data during training, which will affect the accuracy of the final evaluation results. It is a common practice to separate a portion of the training data into validation data to evaluate the training effect of the model.
The validation data is taken from the training data, but is not involved in the training, so that the model can be evaluated relatively objectively to match the data outside the training set. The evaluation of the model in the validation data is commonly done by cross-validation, also known as round robin validation. It divides the original data into K groups (K-Fold), and makes each subset of data into a validation set separately, and the rest of the K-1 subset data as the training set, so that K models will be obtained. These K models are evaluated separately in the validation set, and the final error MSE (Mean Squared Error) is summed and averaged to obtain the cross-validation error. Cross-validation makes effective use of the limited data and the evaluation results are as close as possible to the performance of the models on the test set, which can be used as an indicator for model optimization.
...
After the 5-fold cross validation, we found that the best lambda is about $e^{-3.232} = 0.0391$ in the optimal model. 