...
```{r}
set.seed(2022)
cv.lasso1 <- cv.glmnet(x1, y, alpha = 1, family = "binomial", trControl = ctrl)
plot(cv.lasso1)
lambda1 <- cv.lasso1$lambda.min

set.seed(2022)
cv.lasso2 <- cv.glmnet(x2, y, alpha = 1, family = "binomial", trControl = ctrl)
plot(cv.lasso2)
lambda2 <- cv.lasso2$lambda.min

set.seed(2022)
cv.lasso3 <- cv.glmnet(x3, y, alpha = 1, family = "binomial", trControl = ctrl)
plot(cv.lasso3)
